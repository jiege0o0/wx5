		...
		// console.log(res)
			...
			for (let i = 0; i < totalGroup.length; i++) {
				...
				// {
					// totalGroup[i].level += '%'
				// }
				if(clientData.key == 'score')
				{
					totalGroup[i].level = ''+totalGroup[i].level+' 分'
				}
				else
				{
					var cd1 = Math.floor(totalGroup[i].level/1000)
					var cd2 = Math.floor(totalGroup[i].level/1000*100)%100
					totalGroup[i].level = util.getStringBySecond(cd1).substr(-5) + '′' + ('00' + cd2).substr(-2) + '″';
				}
			}
